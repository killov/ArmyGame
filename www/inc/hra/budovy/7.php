...
foreach($urovne as $key => $uroven){
    echo "<tr><th>".$lang_vyzkum[$key-1]." (".$lang[31].": ".$user->data["v".$key].")</th></tr>";
    echo "<tr><td>";
    if($hodnoty["budovy"][$key]["maximum"] >= $uroven){
        if($cena = $user->vyzkum_cena($key,$uroven)){
            echo "<span class=\"surovina0\">".$cena."</span> ";
        }
        echo "<span class=\"cas\">".cas($user->vyzkum_cas($mesto->data["b7"],$key,$uroven))."</span>";
